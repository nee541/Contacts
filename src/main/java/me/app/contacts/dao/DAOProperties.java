...
/**
 * This class immediately loads the DAO dao.properties file 'dao.dao.properties' once in memory and provides
 * a constructor which takes the specific key which is to be used as property key prefix of the DAO
 * dao.properties file. There is a property getter which only returns the property prefixed with
...
public class DAOProperties {
    ...
    static {
        ...
        try {
            ...
            throw new DAOConfigurationException(
                    "Cannot load dao.properties file '" + PROPERTIES_FILE + "'.", e);
        }
    }
     ...
     * key prefix of the DAO dao.properties file.
     * @param specificKey The specific key which is to be used as property key prefix.
     * @throws DAOConfigurationException During class initialization if the DAO dao.properties file is
        ...
        if (property == null || property.trim().length() == 0) {
            if (mandatory) {
                throw new DAOConfigurationException("Required property '" + fullKey + "'"
                        + " is missing in dao.properties file '" + PROPERTIES_FILE + "'.");
            } else {
